...
skill("sm", "技能: 根据负面属性多少获得相应的盾。冷却150秒", "受虐狂") {
    ...
    if(unit.hasEffect(StatusEffects.unmoving)){
    	numEffects++
    }
    
    if(unit.hasEffect(StatusEffects.sporeSlowed)){
    	numEffects++
    }
    if(unit.hasEffect(StatusEffects.freezing)){
    	numEffects++
    }
    if(unit.hasEffect(StatusEffects.tarred)){
    	numEffects++
    }
    ...
    if(unit.hasEffect(StatusEffects.disarmed)){
    	numEffects++
    }
    
   unit.shield = unit.shield + numEffects * unit.health * 1.2f
