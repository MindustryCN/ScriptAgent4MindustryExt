...
val expireTimeSwitch: Duration by config.key(Duration.ofMinutes(10), "随机绑定码到期时间")
...
fun generate(qq: Long): Int {
    ...
    } while (!map.add(expireTimeSwitch.toMillis(), code, qq))
    return code
}
...
command("switchCode", "管理指令: 为用户生成随机换号码") {
    usage = "<qq>"
    permission = "wayzer.user.switchCode"
    body {
        val qq = arg.firstOrNull()?.toLongOrNull() ?: returnReply("[red]请输入正确的qq号".with())
        reply("[green]换号码{code},有效期:{expireTimeSwitch}".with("code" to generate(qq), "expireTimeSwitch" to expireTimeSwitch))
    }
}
...
command("switch", "切换用户") {
    ...
    body {
        val qq = arg.firstOrNull()?.toIntOrNull()?.let(::check)
            ?: returnReply("[red]请输入正确的6位切换码,如没有，可找管理员获取".with())
        ...
        reply("[green]切换账号[yellow]{qq}[green]成功.".with("qq" to qq))
    }
}
